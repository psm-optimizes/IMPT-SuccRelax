...
def SuccRelaxbySlack(DoseMat, beamlets, Tmax, Tmin, M, UD, UV, iterations, patientID):
    '''
    ...
    for struct in list(DoseMat.keys()):
            if struct != 'Target':
                NumRelaxedCons[struct] = math.floor(UV[struct]*dimensions[f'n_{struct}'])
    ...
    for iter in range(1, iterations):
        ...
        # Create the dictionary of sorted shadow prices
        Shadow_Prices_Dict = {}
        for struct in list(DoseMat.keys()):
            if struct != 'Target':
                Shadow_Prices = []
        ...
        if iter == 1:
            ...
            for struct in list(DoseMat.keys()):
                if struct != 'Target':
                    print(f'No. of {struct} voxels to be relaxed:', NumRelaxedCons[struct])
                    RelaxedConstr[struct] = []
                    for constr_name, _ in Shadow_Prices_Dict[struct][ : NumRelaxedCons[struct]]: 
                        constr = model.getConstrByName(constr_name)
                        constr.setAttr(GRB.Attr.RHS, M)
                        RelaxedConstr[struct].append(constr_name)

                    for constr_name, _ in Shadow_Prices_Dict[struct][NumRelaxedCons[struct] : ]: 
                        constr = model.getConstrByName(constr_name)
                        constr.setAttr(GRB.Attr.RHS, UD[struct])
            
            RelaxedConstrsMemo = RelaxedConstr.copy()
            ...
            for struct in list(DoseMat.keys()):
                if struct != 'Target':
                    for constr_name in RelaxedConstrsMemo[struct]:
                        constr = model.getConstrByName(constr_name)
                        slackVar = constr.getAttr(GRB.Attr.Slack)
                        if slackVar == 0:
                            pass
                        else: 
                            RelaxedConstrsMemo[struct].remove(constr_name)                            

                    print(f'len(RelaxedConstrsMemo[{struct}]) after Slack Analysis:',len(RelaxedConstrsMemo[struct]))
                    NumToRelax[struct] = NumRelaxedCons[struct] - len(RelaxedConstrsMemo[struct])

            # Select the top constraints based on their shadow prices and change their RHS 
            RelaxedConstr = {}
            for struct in list(DoseMat.keys()):
                if struct != 'Target':
                    print(f'No. of {struct} voxels to be relaxed:', NumToRelax[struct])
                    RelaxedConstr[struct] = RelaxedConstrsMemo[struct].copy()
                    print(f'Pre-len(RelaxedConstr[{struct}]):', len(RelaxedConstr[struct]))

                    for constr_name, _ in Shadow_Prices_Dict[struct][:NumRelaxedCons[struct]]:

                        if constr_name in RelaxedConstr[struct]:
                            pass
                        else:
                            RelaxedConstr[struct].append(constr_name)

                        if len(RelaxedConstr[struct]) == NumRelaxedCons[struct]:
                                break
                    print(f"Post-len(RelaxedConstr[{struct}]):", len(RelaxedConstr[struct]))    
                        
                    for constr_name, _ in Shadow_Prices_Dict[struct]:
                        if constr_name in RelaxedConstr[struct]:
                            constr = model.getConstrByName(constr_name)
                            constr.setAttr(GRB.Attr.RHS, M)
                        else:
                    ...
                    print(f'len(Set(RelaxedConstr[{struct}])):',len(set(RelaxedConstr[struct])))
